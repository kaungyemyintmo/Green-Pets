...
document.addEventListener("DOMContentLoaded", function () {
  const callbackUserPets = (responseStatus, responseData) => {
    ...
    if (responseStatus == 404) {
      myPetList.innerHTML = `
        <div class= "container text-center rounded bg-white" style=" padding-top:20px">
        <h5> You have not bought any pets. Adopt pets now! </h5>

        <div class= "container d-flex justify-content-center rounded bg-white" style="padding:20px">

        <button type="button" class="btn btn-primary center " onclick="location.href = 'pet.html';">Adopt Pets </button>
      </div>
      ...
      <img src=${petImgAry[pet.store_id - 1]} class="card-img-top" alt="pet image">
          <p class="card-text">
             ${pet.pet_name}, id : ${pet.pet_id}
          </p>
  </div>
  <a href="userPet.html?pet_id=${pet.pet_id}" class="btn btn-primary">DUTY</a>
      `;
        myPetList.appendChild(displayItem)
      });
    }

  }

  fetchMethod(currentUrl + `/api/pet/users`, callbackUserPets, "GET", data = null, token)
})

// to display user info 
document.addEventListener("DOMContentLoaded", function () {
  const profileCallback = (responseStatus, responseData) => {
    console.log("responseStatus:", responseStatus);
    console.log("responseData:", responseData);
    ...
    <section class="prompts bg-white rounded" style="padding:50px">
    <div class = "col">
    <div class = "container text-center justify-content-center rounded bg-success text-light custom">
     username : ${responseData.username} <br> 
    </div>
    <div class = "container text-center justify-content-center rounded bg-success text-light custom">
    profile id : ${responseData.user_id} <br> 
   </div>
    <div class = "container text-center  justify-content-center rounded bg-success text-light custom">
    email : ${responseData.email} <br>    </div>
    <div class = "container text-center  justify-content-center rounded bg-success text-light custom">
    points : ${responseData.total_points} <br>    </div>
    <div class = "container text-center  justify-content-center rounded bg-success text-light custom">
    trivia progress : ${responseData.trivia_progress}    </div>
    </section> 
    `
    }
  }
  ...
  fetchMethod(currentUrl + `/api/user/details`, profileCallback, "GET", data = null, token)
